...
def run_subagent(prompt: str, agent_type: str = "Explore") -> str:
    ...
    for _ in range(30):
        ...
        sub_msgs.append({"role": "assistant", "content": assistant_msg.content, "tool_calls": assistant_msg.tool_calls})
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        if notifs:
            ...
            messages.append({"role": "assistant", "content": "Noted background results.", "tool_calls": None})
        # s10: check lead inbox
        inbox = BUS.read_inbox("lead")
        if inbox:
            messages.append({"role": "user", "content": f"<inbox>{json.dumps(inbox, indent=2)}</inbox>"})
            messages.append({"role": "assistant", "content": "Noted inbox messages.", "tool_calls": None})
        # LLM call
        response = client.chat.completions.create(
